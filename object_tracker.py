...
encoder = gdet.create_box_encoder(model_filename, batch_size=1) # feature generation
...
while True:
    ...
    cv2.putText(img, "Current frame Vehicle Count: " + str(current_count), (0, 80), 0, 1, (0, 0, 255), 2)
    ...
    D = 700
    # print(pixel[0])
    focal_length = (pixel[0] * D)/width_veh
    print("focal_len: ",focal_length)
    distance_measured = (width_veh * focal_length)/pixel[0]
    # print("distance:",distance_measured)
    # print(center_x)
    if focal_length <100:
        if center_x > 256 and center_x < 768:
            print("Vehicle in front")
            if center_x > 256 and center_x < 512:
                print("Move Right")
            else:
                print("Move left")
        else:
    ...
    if len(ids) == len(ids_pre):
        for i in range(len(ids)):
            if ids[i] == ids_pre[i]:
                oute.append(b[i] - x_y[i])
                d1.append(ids[i])
    b = x_y # took previous
    ids_pre = ids
    x_y = []
    ...
    for i in range(len(oute)):
        if oute[i] > 1 or oute[i]< -20:
            print("car - {} is moving".format(d1[i]))
        else:
            print("car - {} is static".format(d1[i]))
